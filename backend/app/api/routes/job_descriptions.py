...
def create_job_description(
    ...
    return service.create_job_description(payload)


@router.get("", response_model=list[JobDescriptionRead])
def list_job_descriptions(db: Session = Depends(get_db)) -> list[JobDescriptionRead]:
    service = JobAnalysisService(db)
...
def analyze_job_description(
    ...
    try:
        return service.analyze_job_description(job_description_id)
