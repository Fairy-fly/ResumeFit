...
class InterviewQuestionValidationError(RuntimeError):
    pass
...
class InterviewQuestionService:
    ...
    def create_interview_question_result(
        ...
        *,
        ...
        return self._to_read_model(interview_question_result)
    ...
    def _load_context(self, resume_version_id: int, *, user_id: int) -> dict[str, object]:
        ...
        if resume_version.match_report_id is None:
            raise InterviewQuestionValidationError("Resume version does not have a match report context.")
        ...
        if job_analysis is None or job_analysis.job_description_id != job_description.id:
            raise InterviewQuestionNotFoundError("Job analysis was not found.")
        ...
        return {
            ...
            "truth_check_result": truth_check_results[0] if truth_check_results else None,
        }
        ...
        *,
        ...
        if resume_version.resume_profile_id != match_report.resume_profile_id:
            raise InterviewQuestionValidationError(
                "Resume version does not match the linked match report resume."
            )
        if resume_version.job_description_id != match_report.job_description_id:
            raise InterviewQuestionValidationError(
                "Resume version does not match the linked match report job description."
            )
    ...
    def _build_user_prompt(
        ...
        *,
        ...
        match_report: MatchReport,
        truth_check_result: TruthCheckResult | None,
    ) -> str:
        payload = {
            "resume_version": {
            ...
            "truth_check_result": self._truth_check_to_prompt(truth_check_result),
        }
    ...
    def _truth_check_to_prompt(self, truth_check_result: TruthCheckResult | None) -> dict[str, object] | None:
        if truth_check_result is None:
            return None
        return {
    ...
    def _to_read_model(self, interview_question_result: InterviewQuestionResult) -> InterviewQuestionResultRead:
        return InterviewQuestionResultRead(
            id=interview_question_result.id,
            user_id=interview_question_result.user_id,
            resume_version_id=interview_question_result.resume_version_id,
            questions=[
                item if isinstance(item, dict) else {}
                for item in self._from_json_list_of_objects(interview_question_result.questions_json)
            ],
            summary=interview_question_result.summary,
            model_name=interview_question_result.model_name,
            created_at=interview_question_result.created_at,
        )
