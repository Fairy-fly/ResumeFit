...
V0.6 管理后台继续复用 `users` 表，新增 `role` 字段识别管理员。轻量迁移会为旧 SQLite 数据库补齐 `role VARCHAR NOT NULL DEFAULT 'user'`，已有用户默认是普通用户。管理员账号不自动创建，演示或本地开发时可手动把指定用户 `role` 设置为 `admin`。
