...
V0.6 已按“轻量管理后台基础版”方向实现，用于后续商业化运营准备，但仍不包含支付、会员套餐、订单、发票或复杂权限系统。

已完成能力：

- `users` 表新增 `role` 字段，支持 `user` / `admin`。
- 新增 admin 权限依赖，普通用户访问 `/admin/*` 返回 `403`。
- `/auth/me`、注册和登录响应包含 `role`，前端可据此显示或隐藏管理后台入口。
- 新增 `/admin` 前端页面。
- 新增后台接口：
  - `GET /admin/users`
...
- 管理员可以查看用户列表、搜索用户、查看用户详情、查看全站 AI 用量概览。
- 管理员可以启用或禁用普通用户。
- 禁用用户不能登录，已有 token 也不能继续调用业务接口。

V0.6 能力边界：

- 不展示用户简历正文、项目正文、JD 原文或 AI 生成内容。
- 不返回 `password_hash`、token 或任何密钥。
- 不允许管理员禁用自己的账号。
- 不做支付、会员套餐、订单、发票、复杂 RBAC、操作审计、删除用户、修改密码、头像上传、招聘网站爬取或 PDF/Word 导出。
