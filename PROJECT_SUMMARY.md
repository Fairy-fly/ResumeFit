...
- 用量基础设施：V0.4 已支持 AI 调用日志、基础月额度、用量统计和 `/usage` 页面。
...
V0.4 为后续商业化补充用量基础设施，但不实现支付、会员套餐、订单、管理员后台或发票：

- 后端记录每次 AI 调用的用户、功能类型、模型、成功/失败、错误信息和创建时间。
- token usage 和 estimated_cost 字段预留，供应商未返回时允许为空。
- 默认月额度通过 `.env` 中的 `AI_MONTHLY_CALL_LIMIT` 配置。
- 超过额度后阻止 AI 调用并返回明确错误。
- 前端新增 `/usage` 页面，展示本月调用次数、剩余额度、各功能调用次数和最近调用记录。
